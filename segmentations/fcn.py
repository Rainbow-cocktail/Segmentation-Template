...
class FCN(EncoderDecoder):
    def __init__(self, encoder_name='resnet50', in_channel_nb=3, classes_nb=2):
        ...
        # 把 高光谱适配后的 encoder 和 decoder 传递进去
...
if __name__ == '__main__':
    ...
    #     in_channel_nb: 25
    #     classes_nb: 2
    #  因此，这里的输入通道数为 25, 输出类别数为 2
    model = FCN(encoder_name='resnet50', in_channel_nb=25, classes_nb=10)
    model.to('cuda:0')
    output = model(test_im)
    print("model:", model.name)
    print("input to output", test_im.shape, output.size())  # torch.Size([1, 2, 160, 160])

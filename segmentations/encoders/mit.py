...
# 以 (batch_size = 4 , channels = 3 , H = 512 , W = 512) 图像为例
...
class OverlapPatchEmbed(nn.Module):
    """把输入图像转换为 Transformer 可处理的 Token 序列"""

    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        patch_size = (patch_size, patch_size)
        # 将输入的 3 通道图像转换为 768 维的嵌入表示
    ...
    def forward(self, x):
        x = self.proj(x)  # (4, 3, 512, 512) -> (4, 768, 128, 128)
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)  # (4, 768, 128, 128) -> (4, 128*128, 768) (batch_size, num_patches, embed_dim)
        x = self.norm(x)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        """
        缩减查询（SR, Spatial Reduction）注意力机制
        如果 sr_ratio > 1，就对 key-value 进行降维，以减少计算量。
        """
        super().__init__()
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape  # x: (batch, num_patches, embed_dim) = (4, 16384, 768)
        # q: (batch, num_patches, embed_dim) -> (batch, num_patches, num_heads, embed_dim//num_heads) -> (batch, num_heads, num_patches, embed_dim//num_heads)
        # q: (4, 16384, 768) -> (4, 16384, 8, 96) -> (4, 8, 16384, 96 = 768//8)
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)  # 变成 (B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)  # 变成 (B, H*W, C)
            x_ = self.norm(x_)
...
class DWConv(nn.Module):
    """
    深度可分离卷积
    这个模块用于在 Transformer 计算前 进行 局部信息增强，增加 位置感知能力。
    由于是 深度卷积，计算量 远低于普通 3x3 卷积，适合 高效特征提取
    """
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape  # x: (batch_size, num_patches, embed_dim)
        x = x.transpose(1, 2).view(B, C, H, W)  # 变回 (B, C, H, W)
        x = self.dwconv(x)  # 进行深度可分离卷积
        x = x.flatten(2).transpose(1, 2)  # 重新转换回 (B, N, C)
...
class Mlp(nn.Module):
    ...
    def forward(self, x, H, W):
        x = self.fc1(x)  # (B, N, in_features) -> (B, N, hidden_features)
        x = self.dwconv(x, H, W)  # (B, N, hidden_features) -> 经过深度卷积增强特征
        x = self.act(x)
        x = self.drop(x)
...
class MixVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #----------------------------------#
        #   block1
        # (4,3,512,512) -> (4, 16384, 32) -> (4, 16384, 32) -> (4, 32, 128, 128)
        #----------------------------------#
        ...
        #----------------------------------#
        #   block2
        # (4, 32, 128, 128) -> (4, 64, 64, 64) -> (4, 4096, 64) -> (4, 64, 64, 64)
        #----------------------------------#
        ...
        #----------------------------------#
        #   block3
        # (4, 64, 64, 64) -> (4, 160, 32, 32) -> (4, 1024, 160) -> (4, 160, 32, 32)
        #----------------------------------#
        ...
        #----------------------------------#
        #   block4
        # (4, 160, 32, 32) -> (4, 256, 16, 16) -> (4, 256, 256) -> (4, 256, 16, 16)
        #----------------------------------#
...
# FIXME: params并没有想FCN一样真正用于初始化关键的encoder参数，这里更像是占位
mit_encoders = {
    'mit_b0': {'encoder': mit_b0, 'out_shapes': [32, 64, 160, 256], 'params': {'pretrained': False}},
    'mit_b1': {'encoder': mit_b1, 'out_shapes': [64, 128, 320, 512], 'params': {'pretrained': False}},
    'mit_b2': {'encoder': mit_b2, 'out_shapes': [64, 128, 320, 512], 'params': {'pretrained': False}},
    'mit_b3': {'encoder': mit_b3, 'out_shapes': [64, 128, 320, 512], 'params': {'pretrained': False}},
    'mit_b4': {'encoder': mit_b4, 'out_shapes': [64, 128, 320, 512], 'params': {'pretrained': False}},
    'mit_b5': {'encoder': mit_b5, 'out_shapes': [64, 128, 320, 512], 'params': {'pretrained': False}},
}

if __name__ == "__main__":
    # 创建 `mit_b0` 编码器
    encoder = mit_b0(input_channel=3, pretrained=False)  # 关闭预训练

    # 创建测试输入图像 (batch_size=4, channels=3, height=512, width=512)
    input_tensor = torch.randn(4, 3, 512, 512)  # 生成随机图像
